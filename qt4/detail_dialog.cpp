...
DetailDialog::createPlayerLabels()
{
    ...
    layout->addWidget( new QLabel( tr( "    " ) ), row, 0, Qt::AlignRight );
    M_player_rel_pos =  new QLabel( tr( "   0.00,   0.00" ) );
    layout->addWidget( M_player_rel_pos, row, 1, Qt::AlignRight );
    M_player_polar_rel_pos = new QLabel( tr( " (  0.000,    0.0)" ) );
    layout->addWidget( M_player_polar_rel_pos, row, 2, Qt::AlignRight );
    ++row;
...
DetailDialog::updateLabels()
{
    ...
    // relative pos
    Vector2D rel_pos( ball.x_ - player.x_, ball.y_ - player.y_);
    rel_pos.rotate(-player.body_);
    snprintf( buf, 64, " %6.2f, %6.2f",
              rel_pos.x, rel_pos.y );
    M_player_rel_pos->setText( QString::fromAscii( buf ) );
    snprintf( buf, 64, " (%7.3f, %6.1f)",
			  rel_pos.r(), rel_pos.th().degree() );
	M_player_polar_rel_pos->setText( QString::fromAscii( buf ) );
